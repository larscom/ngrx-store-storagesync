...
describe('StorageSync', () => {
  ...
  it('should remove item from storage if version is present in storage but not in config', () => {
    ...
    storage.setItem('ngrx-store-storagesync.version', String(1))
    ...
    expect(storage.getItem('ngrx-store-storagesync.version')).toEqual('1')

    metaReducer(reducer)(initialState, { type: 'ANY_ACTION' })

    expect(storage.getItem('ngrx-store-storagesync.version')).toBeNull()
  })
